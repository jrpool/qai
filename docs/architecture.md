...
The architecture of the current release, version 0.3.0, is an HTML website with a Node.js server handling routing, request handling, and form rendering. A tutorial page links to a comment page, with submission disabled until submissions are handled, and gives instructions for submission of comments as GitHub issues. The application uses the [deployment infrastructure of Kilotest](https://github.com/jrpool/kilotest/blob/main/SERVICE.md). The health of the deployed application is externally monitored.
...
- Comments: Flow from Node.js server to origin repository.
...
- The tutorial page will contain a link to the Issues feature on the origin repository for comment submission.
...
- Submission of the comment form will be disabled because of incomplete submission handling.
